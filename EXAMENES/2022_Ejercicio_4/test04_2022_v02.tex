...
\element{test1}{
\begin{question}{p9}
Para realizar un modelo de elementos finitos de un s\'olido el\'astico 2D que trabaja esencialmente a flexi\'on, que formulaci\'on de elementos no se deber\'ia utilizar:
\begin{multicols}{2}
\begin{choices}
		\correctchoice{Tri\'angulos de tres nodos}
		\wrongchoice{Cuadril\'ateros de cuatro nodos con modos incompatibles.}
		\wrongchoice{Cuadril\'ateros isoparam\'etricos de $8$ nodos.}
		\wrongchoice{Cuadril\'ateros de cuatro nodos con integraci\'on selectiva reducida.}
\end{choices}
\end{multicols}
\end{question}
}
...
\element{test1}{
\begin{question}{p10}
En este ejercicio, la diferencia entre los resultados obtenidos con las
distintas formulaciones de elementos de $8$ nodos no es muy apreciable.
El motivo es:
\begin{multicols}{2}
\begin{choices}
        \correctchoice{No hay elementos de poco espesor cuyos modos de flexión
intervengan de manera significativa.}
        \wrongchoice{El orden de la interpolación de los desplazamientos
es bajo (funciones de forma trilineales), frente a otros elementos que
podrían utilizarse con funciones de forma cuadráticas o cúbicas}
        \wrongchoice{La malla es muy gruesa}
        \wrongchoice{El problema es lineal sin que se produzcan grandes
deformaciones}
\end{choices}
\end{multicols}
\end{question}
